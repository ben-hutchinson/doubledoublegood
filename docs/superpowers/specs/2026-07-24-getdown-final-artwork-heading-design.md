...
- Add the panel heading `GETDOWN SERVICES INSTORE` through the typed `homePurchaseFeature` content.
- Change the note exactly to: “Purchase the LP to receive free entry for one named person to the Getdown Services gig instore”.

## Layout

Render the content inside the carousel header in this order:

1. `GETDOWN SERVICES INSTORE` heading.
2. Revised free-entry note.
...
- Update homepage tests first to require the exact heading, note, new image path, alt text, and DOM order.
